...
class PDFProvider(nn.Module):
    ...
    def _init_parametric(self):
        self.params = nn.Parameter(torch.tensor([
            ...
            [0.2,  -0.2, 7.0, -1.5, 1.0],
            [0.1,  -0.2, 8.0, -2.0, 1.5],
            [0.02, 0.0, 10.0, 0.0, 0.0],
            [0.005, 0.5, 12.0, 0.0, 0.0],
...
class MatrixElements:
    ...
    def k_factor_gg_higgs(mH: float, pt: float = 0.0, rapidity: float = 0.0) -> float:
        # pT‑ and rapidity‑dependent NLO/ NNLO‑inspired correction
        base = 1.1 + 0.2 * (mH - 125.0) / 10.0
        pt_factor = 1.0 + 0.05 * math.log(1 + pt / 50.0) if pt > 0 else 1.0
        rap_factor = 1.0 + 0.02 * abs(rapidity)
        return base * pt_factor * rap_factor
    ...
    def gg_higgs_partonic(self, s_hat: torch.Tensor, mH: float = 125.0,
        ...
        return lo * self.k_factor_gg_higgs(mH, pt, rap)
...
class NASADataLoader:
    ...
    def load_planck_highl_spectrum(filepath: Optional[str] = None):
        ...
        # Robust reading: first part data, then covariance block if present
        with open(filepath, 'r') as f:
            lines = f.readlines()
        # Find blank line or start of covariance
        cov_start = None
        for i, line in enumerate(lines[1:], start=1):  # skip header line
            if line.strip() == "":
                cov_start = i+1  # next line
                break
        if cov_start is None:
            # assume no covariance, just data
            data = np.loadtxt(filepath, skiprows=1)
            ell = torch.tensor(data[:,0], dtype=torch.float32)
            Dl  = torch.tensor(data[:,1], dtype=torch.float32)
            # use diagonal errors if third column present, else ones
            if data.shape[1] >= 3:
                err = torch.tensor(data[:,2], dtype=torch.float32)
            else:
                err = torch.ones_like(ell) * 0.1 * Dl
            cov = torch.diag(err**2)
        else:
            data = np.loadtxt(filepath, skiprows=1, max_rows=cov_start-2)
            ell = torch.tensor(data[:,0], dtype=torch.float32)
            Dl  = torch.tensor(data[:,1], dtype=torch.float32)
            try:
                ...
                if cov_data.ndim == 2:
                    ...
                    # single line with diagonal? Assume errors squared
                    err = torch.tensor(data[:,2], dtype=torch.float32) if data.shape[1] >= 3 else torch.ones_like(ell) * 0.1 * Dl
                    cov = torch.diag(err**2)
            except:
                err = torch.tensor(data[:,2], dtype=torch.float32) if data.shape[1] >= 3 else torch.ones_like(ell) * 0.1 * Dl
                cov = torch.diag(err**2)
...
class HuWhiteCMB(CMBBackend):
    """Hu & White (1997) analytic CMB spectrum."""
    def __init__(self, cosmo: Cosmology, lmax: int = 2500, device='cpu'):
...
class CosmoPowerCMB(CMBBackend):
    ...
    def __init__(self, cosmo: Cosmology, lmax: int = 2500, device='cpu',
        ...
        if model_path is None:
            ...
            logger.warning("CosmoPower model not specified; using default PCA emulator.")
        else:
...
# =============================================================================
# 10. Differentiable Generators (structural + external event generators)
# =============================================================================
...
# ---- External Event Generators (optional) ----------------------------------
...
class CMBLikelihood:
    """Gaussian CMB likelihood with full covariance, using Cholesky for numerical stability."""
    def __init__(self, cmb_calculator: CMBBackend, ell_data, Cl_data, cov):
        ...
        if info != 0:
            # fallback to adding a small ridge
    ...
    def log_likelihood(self, A_s, n_s, tau):
        ...
        # solve L y = delta, then chi2 = y^T y
...
# =============================================================================
# 12. Full Frequentist Analysis (with multi‑start & bootstrap calibration)
# =============================================================================
class FrequentistAnalysis:
    ...
    def fit(self, freeze: Dict[str, float] = None, max_iter: int = 500, n_restarts: int = 1) -> float:
        """Returns minimum NLL after optional multi‑start."""
        best_nll = float('inf')
        original = {n: p.data.clone() for n, p in self._param_dict.items()}
        for restart in range(n_restarts):
            if restart > 0:
                # random perturbation of free parameters
                for n, p in self._param_dict.items():
                    if p.requires_grad:
    ...
    def bootstrap_calibrated_pvalue(self, null_poi_name: str, null_value: float,
                                    n_toys: int = 500, pdf_sampler: Callable = None) -> float:
        """Bootstrap calibration of p‑value using the test statistic under the null."""
        if pdf_sampler is None:
            raise ValueError("pdf_sampler must be provided to generate toy data.")
        # Fit null model to get test statistic on observed data
        nll_null_obs = self.conditional_fit(null_poi_name, null_value)
        nll_best_obs = self.unconditional_fit()
        q_obs = max(0.0, 2 * (nll_null_obs - nll_best_obs))
        # Now generate toys under null
        self.fit(freeze={null_poi_name: null_value})  # set null values
        q_toys = []
        for _ in range(n_toys):
            toy_data = pdf_sampler()  # generate data from the null model
            # Create a temporary likelihood with toy_data
            # We assume that the likelihood can be re-instantiated with new data.
            # For simplicity, we create a new StructuralLikelihood with the same generator but evaluated on toy_data.
            # However, we need access to the generator. We'll store it in the class.
            toy_nll = -torch.sum(torch.log(self.generator.pdf(toy_data).clamp(1e-12)))
            # We need to compare with best fit on toy data. We'll do a quick unconditional fit on toy.
            # This is a simplified implementation; a full version would require re-fitting.
            # For demonstration we leave a placeholder.
            # In practice, you would create a new FrequentistAnalysis with the toy data.
            pass
        return np.mean(np.array(q_toys) >= q_obs)


# =============================================================================
# 13. Bayesian Analysis (Adaptive MH, NUTS, Laplace, Bayes factors)
# =============================================================================
class BayesianAnalysis:
    ...
    def sample_mh(self, n_samples=5000, burn_in=1000, step_size=0.1,
        ...
        """Adaptive Metropolis‑Hastings with covariance estimation."""
        if init is None:
        ...
        for i in range(n_samples + burn_in):
            ...
            if adapt_cov and i > 100 and (i - burn_in) % 50 == 0:
                # update covariance estimate using accepted points after burn-in
                if n_accepted > 1:
                    accepted_points = []
