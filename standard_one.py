...
# Collider event generators (optional)
try:
    import pythia8
    HAS_PYTHIA = True
except ImportError:
...
class CMBBackend(nn.Module):
    """Abstract base for a differentiable CMB TT calculator."""
...
class CAMBCMB(CMBBackend):
    ...
    def C_ell_at(self, ell, A_s, n_s, tau):
        ...
        return torch.interp(ell.to(self.device), full_ell, full_Cl)
...
class ClassCMB(CMBBackend):
    ...
    def C_ell_TT(self, A_s=2.1e-9, n_s=0.96, tau=0.054):
        ...
        ell = torch.arange(2, self.lmax+1, device=self.device, dtype=torch.float32)
        Cl = torch.tensor(cls['tt'][2:self.lmax+1], device=self.device) * 1e12  # CLASS returns dimensionless
...
class Pythia8Generator:
    """Wrapper for Pythia8 (GPL‑2). Usage implies GPL compliance."""
...
class HerwigGenerator:
    """Wrapper for Herwig (GPL‑3). Usage implies GPL compliance."""
    ...
    def generate_events(self, n: int) -> List[Dict]:
        ...
        for _ in range(n):
            self.herwig.next()
            # Particle extraction similar to Pythia example
            events.append([])
        return events
...
# =============================================================================
# 12. Full Frequentist Analysis (with bootstrap calibration)
# =============================================================================
class FrequentistAnalysis:
    """
    Full frequentist inference using profile likelihood ratio.
    Provides asymptotic significance, p‑values, confidence intervals,
    and a bootstrap‑calibrated alternative valid for small samples.
    """
    ...
    def _bootstrap_test_statistic(self, null_poi_name: str, null_value: float,
                                  n_toys: int, pdf_sampler: Callable) -> np.ndarray:
        """Generate toys under the null hypothesis and return array of q0 values."""
        q0_toys = np.empty(n_toys)
        # Fit null model
        self.fit(freeze={null_poi_name: null_value})
        # Sample toy data
        for i in range(n_toys):
            toy_data = pdf_sampler()
            # We need to evaluate NLL on toy data. This requires a likelihood function
            # that accepts data. Since nll_fn is fixed with current data, we need a
            # version that takes data as argument. We'll extend nll_fn to accept data
            # by constructing a temporary likelihood.
            # For simplicity, we assume the likelihood is an instance of StructuralLikelihood
            # and we can call likelihood(toy_data). This is implemented via
            # FrequentistAnalysisWithData.
            pass
        return q0_toys
