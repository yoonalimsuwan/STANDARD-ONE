...
class PDFProvider(nn.Module):
    ...
    def _init_lhapdf_grid(self, pdf_name):
        ...
        self.flavors = list(range(-6, 7))   # PDF grids for each PID
        # Build a 2D grid: (x, Q) -> xf(x,Q) for each flavor
        ...
        # shape: (n_q, n_x, n_flavors)
        grid = torch.tensor(grid_vals, dtype=torch.float32, device=self.device)
        # store as (1, n_flavors, n_q, n_x) for grid_sample with (N,C,H,W)
        self._grid = grid.permute(2, 0, 1).unsqueeze(0)   # [1, n_flavors, n_q, n_x]
        # Normalized coordinates for grid_sample: we need to map (log10 x, log10 Q) to [-1,1]
    ...
    def _init_parametric(self):
        # Improved parametric form inspired by typical global PDF fits.
        ...
        self.params = nn.Parameter(torch.tensor([
            ...
            [0.08, -0.2, 7.0, -1.0, 0.5, 0.1],   # s
            [0.02, -0.1, 9.0, -1.5, 0.6, 0.2],   # c
            [0.005, 0.0,11.0, -2.0, 0.8, 0.3],   # b
    ...
    def _interpolate_lhapdf(self, x: torch.Tensor, flavour: str, Q: float) -> torch.Tensor:
        ...
        # here we want to sample many x at a single Q.  We'll repeat for each x.
        n_pts = x.shape[0]
        # input grid: [1, n_flavors, n_q, n_x] -> select flavor idx
        # create a batch of size n_pts with that flavor
        grid_4d = self._grid[:, idx:idx+1, :, :]   # [1,1,n_q,n_x]
        # grid coordinates: for each point we need (q_norm, x_norm) in shape (N,1,1,2)
        # q_norm is scalar, repeated for each x.
        coords = torch.tensor([q_norm], device=self.device).expand(n_pts)  # [N]
        grid_coords = torch.stack([coords, x_norm], dim=-1).view(n_pts, 1, 1, 2)  # [N,1,1,2]
        sampled = F.grid_sample(grid_4d.expand(n_pts, -1, -1, -1, -1),
...
# =============================================================================
class KFactorProvider:
    """Differentiable K‑factor based on interpolation of realistic data."""
    def __init__(self, process='drell_yan', device='cpu'):
        self.device = device
        if process == 'drell_yan':
            # Approximate NNLO/LO K‑factors for Drell‑Yan at 13 TeV.
            # Table: mass [GeV], K-factor
            _mass = [50, 60, 70, 80, 90, 100, 110, 120, 130, 150, 200, 300, 500, 1000]
            _kval = [1.35, 1.32, 1.30, 1.28, 1.26, 1.25, 1.24, 1.23, 1.23, 1.22, 1.20, 1.18, 1.15, 1.12]
            ...
            # Approximate NNLO/LO K‑factor for gg→H
            _mass = [100, 110, 120, 125, 130, 140, 150]
            _kval = [1.90, 1.85, 1.80, 1.77, 1.75, 1.70, 1.65]
            self.register_buffer('mass_grid', torch.tensor(_mass, dtype=torch.float32))
            self.register_buffer('k_grid', torch.tensor(_kval, dtype=torch.float32))
        else:
    ...
    def __call__(self, mass: torch.Tensor) -> torch.Tensor:
...
class MatrixElements:
    ...
    def k_factor_drell_yan(s_hat: torch.Tensor) -> torch.Tensor:
        # replaced by KFactorProvider; kept for backward compatibility
...
class NASADataLoader:
    ...
    def load_planck_highl_spectrum(filepath: Optional[str] = None):
        ...
        # Robust parser: first block of numbers is data, then optional covariance matrix.
        with open(filepath, 'r') as f:
            lines = f.readlines()
...
class ColliderGenerator(BaseStructuralGenerator):
    ...
    def pdf(self, m):
        ...
        csoc_mod = 1.0 + self.csoc(r) * 0.1   # small perturbation
...
# =============================================================================
# 12. Full Frequentist Analysis (multi‑start, bootstrap calibration)
# =============================================================================
class FrequentistAnalysis:
    ...
    def fit(self, freeze: Dict[str, float] = None, max_iter: int = 500, n_restarts: int = 1) -> float:
        ...
        if n_restarts > 1:
            for n, p in self._param_dict.items():
                p.data = best_state[n]
        # Restore original requires_grad settings (do not blindly set all to True)
        for n, p in self._param_dict.items():
...
class StandardOneUnified:
    ...
    def load_collider_data(self, source='simulate', **kwargs):
        if source == 'simulate':
            ...
            self.use_pyhf = True
            self.data = None   # pyhf uses its own internal data
        else:
    ...
    def run_full_frequentist(self, poi_name: str = 'log_mu', null: float = 0.0,
        ...
        if self.use_pyhf:
            logger.info("pyhf data loaded; run frequentist analysis via run_pyhf_fit() instead.")
    ...
    def run_pyhf_fit(self):
        """Perform a maximum likelihood fit using pyhf and return results."""
        if not HAS_PYHF or self.pyhf_likelihood is None:
            raise RuntimeError("pyhf not available or no pyhf data loaded.")
        pyhf.set_backend('pytorch')
        result = pyhf.infer.mle.fit(self.pyhf_likelihood.data_tensor, self.pyhf_likelihood.model,
                                    return_uncertainties=True)
        bestfit = result[:, 0]
        uncertainties = result[:, 1]
        logger.info(f"pyhf MLE fit: parameters = {bestfit}, uncertainties = {uncertainties}")
        return {'bestfit': bestfit.tolist(), 'uncertainties': uncertainties.tolist()}
