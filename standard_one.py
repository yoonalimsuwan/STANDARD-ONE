...
#   • Differentiable CMB (Hu & White analytic + neural emulator / CosmoPower interface)
...
#   • cosmo‑power (MIT)                — CMB neural emulator (optional pre‑trained)
#
# Business use: MIT licence for this code. External libraries retain their own
# licences. GPL‑licensed LHAPDF may impose restrictions if linked; to avoid
# copyleft, rely on the built‑in parametric PDFs.
#
...
from scipy.stats import linregress       # used only for quick stat, removable
from scipy.interpolate import interp1d   # not used in differentiable graph
...
class ForceParameters:
    """Differentiable running couplings with proper threshold matching.
    α_s follows 2‑loop RGE with continuous n_f(m_c,m_b,m_t)."""
    def __init__(self, device='cpu'):
        # World-average values (PDG 2024)
        self.alpha_EM_MZ = 1 / 127.952
        self.alpha_s_MZ   = 0.1180               # 5‑flavour MSbar at MZ
    ...
    def alpha_EM(self, Q2=None):
        """Constant EM coupling at MZ. QED running negligible for current scope."""
        return torch.tensor(self.alpha_EM_MZ, device=self.device)

    def alpha_s(self, Q2: Union[float, torch.Tensor]) -> torch.Tensor:
        """2‑loop α_s(μ) with smooth n_f(μ) and decoupling at quark masses."""
...
class PDFProvider(nn.Module):
    """Differentiable PDFs via LHAPDF grid or internal parametric form."""
    ...
    def _init_lhapdf_grid(self, pdf_name):
        ...
        self.flavors = list(range(-6, 7))
        self._x_grid = torch.logspace(-5, 0, self.grid_size, device=self.device)
        self._q_grid = torch.logspace(0, 3, 50, device=self.device)
        grid_vals = []
        ...
        self._grid = grid.permute(2, 0, 1).unsqueeze(0).unsqueeze(0)
    ...
    def _init_parametric(self):
        ...
        self.pid_map = {'u':2, 'd':1, 's':3, 'c':4, 'b':5, 't':6,
    ...
    def _interpolate_lhapdf(self, x: torch.Tensor, flavour: str, Q: float) -> torch.Tensor:
        pid = self.pid_map.get(flavour, None)
        if pid is None:
            raise ValueError(f"Unknown flavour: {flavour}")
        idx = self.flavors.index(pid)
        x_log = torch.log10(x.clamp(min=1e-9))
        q_log = torch.log10(torch.tensor(Q, device=self.device).float())
        x_grid_log = torch.log10(self._x_grid)
        ...
        grid = self._grid[:, :, idx:idx+1, :, :]
        coords = torch.stack([x_norm, q_norm.expand_as(x_norm)], dim=-1).view(-1, 1, 1, 2)
        sampled = F.grid_sample(grid.expand(coords.shape[0], -1, -1, -1, -1),
                                coords, mode='bilinear', padding_mode='border',
                                align_corners=True)
    ...
    def luminosity_qqbar(self, sqrts: float, M: torch.Tensor, qtype: str = 'u') -> torch.Tensor:
        ...
        x2 = tau_b / x1
        f_q   = self.xf(x1.flatten(), f'{qtype}_val') + self.xf(x1.flatten(), f'{qtype}_sea')
        f_q   = f_q.view(x1.shape)
        f_qbar = self.xf(x2.flatten(), f'{qtype}_sea').view(x2.shape)
        jac = (1 - tau_b).unsqueeze(0)
...
class MatrixElements:
    ...
    def drell_yan_sigma(self, sqrts: float, M: torch.Tensor) -> torch.Tensor:
        ...
        for q in ['u','d','s','c','b']:
            f_q_val = self.pdf.xf(x1.flatten(), f'{q}_val') + self.pdf.xf(x1.flatten(), f'{q}_sea')
            f_q_val = f_q_val.view(x1.shape)
            f_qbar = self.pdf.xf(x2.flatten(), f'{q}_sea').view(x2.shape)
            sigma_hat = self.drell_yan_partonic(M.unsqueeze(1)**2, flavour=q if q in ['u','c'] else 'd')
            integrand = (f_q_val * f_qbar) * sigma_hat / (x1 * sqrts**2) * (1 - tau_b)
...
# =============================================================================
# 8. Cosmology & Fully Differentiable CMB (Hu & White analytic)
# =============================================================================
class Cosmology:
...
class DifferentiableCMB(nn.Module):
    """
    Fully differentiable CMB TT power spectrum using the Hu & White (1997)
    analytic formula. All computations stay within PyTorch.
    """
    def __init__(self, cosmo: Cosmology, lmax: int = 2500, device='cpu',
        ...
        if self.use_emulator:
            self.emulator = None
            logger.warning("CMB emulator not loaded; falling back to analytic Hu & White.")
    ...
    def _precompute_background(self):
        ...
        self.z_eq = 2.5e4 * Omh2 * theta_cmb**(-4)
        self.k_eq = 7.46e-2 * Omh2 * theta_cmb**(-2)
    ...
    def C_ell_TT(self, A_s: float = 2.1e-9, n_s: float = 0.96,
        ...
        C_ell = _cmb_TT_huwhite(ell, self.rs, self.D_A, self.ell_eq, self.ell_D,
                                self.cosmo.Ob, self.cosmo.Om, A_s, n_s, tau, self.theta_s)
        return ell, C_ell

    def C_ell_at(self, ell: torch.Tensor, A_s: float, n_s: float, tau: float) -> torch.Tensor:
        return _cmb_TT_huwhite(ell, self.rs, self.D_A, self.ell_eq, self.ell_D,
                               self.cosmo.Ob, self.cosmo.Om, A_s, n_s, tau, self.theta_s)


def _cmb_TT_huwhite(ell, rs, DA, ell_eq, ell_D, Ob, Om, A_s, n_s, tau, theta_s):
    x = ell * theta_s
    D_damp = torch.exp(-(ell / ell_D)**1.2)
    omega_b = Ob * (DA / 2997.9 * 100)**2
...
class SemanticStateContraction:
    """Semantic state contraction filter (non‑parametric)."""
...
class CMBLikelihood:
    """Gaussian CMB likelihood with full covariance – theory evaluated directly at data ℓ."""
...
# =============================================================================
# 12. Full Frequentist Analysis (Profile Likelihood Ratio, p‑values, limits)
# =============================================================================
class FrequentistAnalysis:
    """
    Full frequentist inference using profile likelihood ratio.
    Provides significance, p‑values, confidence intervals via Wilks/Chernoff theorems.
    """
    def __init__(self, nll_fn: Callable[[], torch.Tensor],
                 ...
                 device='cpu'):
        """
        nll_fn: a callable with no arguments that returns the negative log‑likelihood.
        params: list of trainable torch.Parameters.
    ...
    def fit(self, freeze: Dict[str, float] = None, max_iter: int = 500) -> float:
        """Minimize NLL with optional frozen parameters, returns minimum NLL."""
        self._set_frozen(freeze)
        free_params = [p for p in self.params if p.requires_grad]
        if not free_params:
            return self.nll_fn().item()
    ...
    def unconditional_fit(self) -> float:
        """Fit all parameters and return NLL at global MLE."""
        return self.fit(freeze=None)

    def conditional_fit(self, poi_name: str, poi_value: float) -> float:
        """Fit with POI fixed, return conditional NLL."""
        return self.fit(freeze={poi_name: poi_value})

    def q0(self, poi_name: str, null: float = 0.0) -> float:
        """Test statistic q0 for discovery (null = 0 for positive signal)."""
    ...
    def significance(self, poi_name: str, null: float = 0.0) -> float:
        """Asymptotic significance Z = sqrt(q0)."""
        return math.sqrt(self.q0(poi_name, null))

    def p_value(self, poi_name: str, null: float = 0.0) -> float:
        """Asymptotic p‑value from Wilks' theorem (half‑chi^2 for q0)."""
        q0 = self.q0(poi_name, null)
        # For discovery with positive signal, distribution is 0.5 δ(0) + 0.5 χ²₁
        if q0 == 0:
            return 1.0
    ...
    def confidence_interval(self, poi_name: str, cl: float = 0.68,
        ...
        """
        Profile likelihood confidence interval for a single POI.
        Returns (lower, upper) bounds at given confidence level.
        """
        delta_nll = 0.5 * chi2.ppf(cl, 1)  # from scipy
        from scipy.stats import chi2
    ...
    def upper_limit(self, poi_name: str, cl: float = 0.95,
        ...
        """Upper limit at 1‑CL (one‑sided). Uses CLs method with q_tilde approximation."""
        from scipy.stats import chi2
        # For upper limit, test statistic q_mu for mu > mu_hat
        # asymptotic distribution: half chi^2 for mu_hat < mu, else 0.
        # We find mu where p_mu = 0.05 (for 95% CL)
        best_nll = self.unconditional_fit()
        best_val = self._param_dict[poi_name].data.item()
        # We compute p_value for each mu using test q_tilde
        # p_mu = 1 - Phi(sqrt(q_mu))
        def p_mu(mu_val):
            # fit conditional on poi = mu_val
            cond_nll = self.conditional_fit(poi_name, mu_val)
            q_mu = 2 * (cond_nll - best_nll)
            if q_mu < 0:
                q_mu = 0.0
            # asymptotic distribution for upper limit: half chi^2 plus delta
            # simplified: p_mu = 0.5 * P(chi2_1 > q_mu) + 0.5 * P(chi2_0 > q_mu)
            # which is 0.5 * (1 - chi2.cdf(q_mu, 1))  (since chi2_0 gives 1 if q_mu=0, else 0)
            # Actually correct: p_mu = 1 - Φ(sqrt(q_mu))
        ...
        grid = torch.linspace(scan_range[0], scan_range[1], n_steps)
        p_vals = [p_mu(v.item()) for v in grid]
        ...
        limit = x0 + (target - y0) * (x1 - x0) / (y1 - y0 + 1e-12)
        return limit


# =============================================================================
# 13. Bayesian Analysis (MH, NUTS, Laplace, Bayes factors)
# =============================================================================
class BayesianAnalysis:
    """Full Bayesian inference with MCMC, Laplace approximation, and Bayes factors."""
    def __init__(self, log_prob_fn: Callable[[], torch.Tensor],
                 params: List[nn.Parameter],
                 param_names: List[str],
                 device='cpu'):
        self.log_prob_fn = log_prob_fn   # returns log posterior (or log likelihood + log prior)
        self.params = params
        self.param_names = param_names
        self.device = device
        self._param_dict = dict(zip(param_names, params))
    ...
    def laplace_approximation(self) -> Dict[str, Tuple[float, float]]:
        """MAP via L‑BFGS + Hessian for Laplace approximation."""
    ...
    def marginal_likelihood_laplace(self) -> float:
        """Log marginal likelihood using Laplace approximation."""
        ...
        k = len(self.params)
        # prior at MAP (flat assumed -> log prior = constant, ignore)
    ...
    def bayes_factor_laplace(self, other_model: 'BayesianAnalysis') -> float:
        """Log Bayes factor this_model / other_model using Laplace approx."""
...
class ModelComparator:
    ...
    def compare(models: Dict[str, nn.Module], data: torch.Tensor,
        ...
        for name, model in models.items():
            # create nll_fn for FrequentistAnalysis
            def make_nll(mod):
            ...
            # Bayes factor via Laplace (model against itself, not meaningful, but we can compute marginal)
...
class StandardOneUnified:
    ...
    def _get_all_params(self):
        return list(self.generator.parameters()) + list(self.csoc.parameters())

    def run_full_frequentist(self, poi_name: str = 'log_mu', null: float = 0.0,
                             cl: float = 0.68):
        """Perform a complete frequentist analysis: significance, p‑value, confidence interval."""
    ...
    def run_full_bayesian(self, n_samples=2000, warmup=500, use_nuts=False):
        ...
        names = self._get_all_param_names()
        def log_prob_fn():
            return -self.structural_likelihood(self.data)  # log likelihood (flat prior)
    ...
    def model_comparison(self, model_list: List[BaseStructuralGenerator]):
        """Compare multiple generator models using AIC, BIC, Bayes factors."""
        models = {f"model_{i}": gen for i, gen in enumerate(model_list)}
        names_all = [n for gen in model_list for n,_ in gen.named_parameters()]
        # For simplicity, use same data; each model will be evaluated with its own likelihood
        results = {}
...
# =============================================================================
def run_tests():
    """Validation against known physics values."""
    ...
    assert xf.item() > 0, "PDF u_val at x=0.1 is zero"
    ...
    cmb = DifferentiableCMB(cosmo, device=dev)
    ell, Cl = cmb.C_ell_TT()
...
def main():
    ...
    if args.frequentist:
        res = framework.run_full_frequentist(poi_name=args.poi, null=args.null, cl=args.cl)
        print(res)
    if args.bayesian:
